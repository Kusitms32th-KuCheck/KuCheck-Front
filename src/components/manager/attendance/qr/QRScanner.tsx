    ...
    <div className="relative h-full w-full max-w-[1260px] overflow-hidden rounded-[12px] bg-black shadow-md">
      ...
      {hasMultipleCameras && (
        <button
          ...
          className="bg-opacity-50 hover:bg-opacity-70 absolute top-4 right-4 z-20 rounded-full bg-black p-3 text-white transition-colors"
          disabled={!scanning}
          aria-label={`${facingMode === 'user' ? '후면' : '전면'} 카메라로 전환`}
        >
