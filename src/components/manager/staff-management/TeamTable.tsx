...
interface TeamTableHandle {
  refreshData: () => void
}

interface TeamTableProps {
  isEditMode?: boolean
  setHandleSaveRoles?: (fn: () => void) => void
}

const TeamTable = forwardRef<TeamTableHandle, TeamTableProps>(
  ({ isEditMode, setHandleSaveRoles }, ref) => {
    const [members, setMembers] = useState<Member[]>([])
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState<string | null>(null)
    const [staffRoleItems, setStaffRoleItems] = useState<{ memberId: number; role: 'STAFF' | 'MANAGEMENT' | 'EXECUTIVE' }[]>([])

    // 권한 변경 내역 관리
    const handleRoleChange = (item: { memberId: number; role: 'STAFF' | 'MANAGEMENT' | 'EXECUTIVE' }) => {
    ...
    // useImperativeHandle로 refreshData 메서드 노출
